...
public class CreateFilesBasedOnState
{
	public static void main(String[] args)
	{
		...
		/*
			The following lines were modified from the originals in the book
			for the following reasons:
			1) Book version fails to compile due to improper Paths.get() call
			2) Revised version from the book publisher throws IOException: file
			not found when trying to create the file.
			3) Cross platform compatability
		*/
		Path inStateName = Paths.get("InStateCusts.txt");
		Path outOfStateName = Paths.get("OutOfStateCusts.txt");

		Path inStateFile = inStateName.toAbsolutePath();
		Path outOfStateFile = outOfStateName.toAbsolutePath();
		...
		// Book example is missing catch statement
		catch(Exception e)
		{
